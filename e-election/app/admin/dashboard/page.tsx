    ...
    try {
      if (contract) {
        ...
        const updatedCandidates = votesData.data.map((candidate: Party) => {
          ...
          return {
            ...candidate,
            DESCRIPTION: `${candidate.DESCRIPTION} (Votes: ${voteCount})`,
          };
        });

        setCandidates(updatedCandidates);

        const voteCounts: { [key: number]: number } = Array.from(
          uniqueElecteurs.entries()
        ).reduce((acc: { [key: number]: number }, [UID, voteCount]) => {
          acc[UID] = voteCount;
          return acc;
        }, {});
        ...
        console.error(
          "Please connect your MetaMask account and connect the contract"
        );
      }
    } catch (error) {
            ...
            >
              ...
              {candidate.PHOTO && (
                <img
                  src={candidate.LOGO}
                  alt={candidate.NAME}
                  className="mt-2 h-20 w-40 object-cover"
                />
              )}

              <p className="mt-2">Party: {candidate.PARTY}</p>
              <p className="mt-2 text-danger">
                Votes: {result[candidate.uid] || 0}
              </p>
            </div>
          ))}
        </div>
